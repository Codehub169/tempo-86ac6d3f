...
// callback: Function to call with error or success result.
const addSearchToHistory = (city, date, time, callback) => {
    if (!db) {
        // This case should ideally not be hit if initDb is called first by server.js
        return callback(new Error("Database not initialized. Call initDb first."));
    }
...
// callback: Function to call with error or the retrieved rows.
